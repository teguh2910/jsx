                    ...
                    <div class="card editor-section">
                        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;">
                            <div>
                                ...
                                <p style="margin:0;color:var(--gray);font-size:13px;">Add approval steps by role from User Management.</p>
                            </div>
                                            ...
                                            <select class="input ed-approver-role" data-id="${a.id}">
                                                ${(() => {
                                                    ...
                                                    if (roleOptions.length === 0) {
                                                        roleOptions.push("spv", "mgr", "gm");
                                                    }
                                                    const selectedRole = a.role || a.title || a.name || roleOptions[0];
                                                    return roleOptions.map(role => `<option value="${role}" ${selectedRole === role ? "selected" : ""}>${role.toUpperCase()}</option>`).join("");
                                                })()}
                                            </select>
                                        </div>
                                    </div>
